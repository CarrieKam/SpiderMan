...
/**
 * Ce composant personnalise permet d'afficher une image accompagnee d'une barre
 * de defilement vertical. Le tout est placee dans un jpanel.
 * 
 * Pour utiliser ce composant: le placer sur l'interface (avec ou sans
 * WindowBuilder). Ensuite, executer sa methode setFichierImage pour associer
 * l'image desiree. Cette derniere methode peut etre appelee a tout moment pour
 * modifier le contenu du panel sur demande.
 * 
 ...
 * (setLargeurCadre) - le nom du fichier associe (setFichierImage)
 * 
...
public class ImageAvecDefilement extends JPanel {
	...
	public void setFichierImage(String nomFichierImage) {
		...
		if (urlImage == null) {
			JOptionPane.showMessageDialog(null,
					"ImageAvecBarre / setImage : Fichier " + nomFichierImage + " introuvable!");
			return;
		}
	...
	/**
	 * Methode servant qui donne les images courantes
	 * 
	 * @param nomFichierImage le nom de l'image
	 * @return nomFichierCourant le nom de l'image
	 */
	//Carrie 
	public String getFichierImage() {
		return this.nomFichierCourant;
	}
